...
{
    fetch('http://localhost/Cac24145Front/api/api.php', 
        {
        method: 'POST',
        headers: {
...
function loadItems() {
  ...
  {
    fetch(`http://localhost/Cac24145Front/api/api.php?id=${id}`, 
        {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json'
...
function updateItem(data) 
    {
        fetch(`http://localhost/Cac24145Front/api/api.php?id=${data.id}`, 
            {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
        ...
        .then(result => {
            ...
            document.getElementById('title_form').innerHTML = "Agregar Pelicula";
        })
        .catch(error => 
            {
            console.error('Error:', error);
  ...
  window.editItem = function(id, titulo, fecha_lanzamiento,genero,duracion,director,reparto,sinopsis) 
  {
    ...
    document.getElementById('title_form').innerHTML = "Editar Datos";
  };
